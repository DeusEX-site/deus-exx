...
// API для получения истории изменений капы
Route::middleware('auth')->get('/api/cap-history/{capId}', function (Request $request, $capId) {
    try {
        $capAnalysisService = new \App\Services\CapAnalysisService();
        $history = $capAnalysisService->getFormattedCapHistory($capId);
...
// API для получения истории изменений по ключу совпадения (аффилейт-брокер-гео)
Route::middleware('auth')->get('/api/cap-history-match', function (Request $request) {
    try {
        $affiliate = $request->get('affiliate', '');
        $broker = $request->get('broker', '');
        ...
        if (empty($affiliate) || empty($broker)) {
            return response()->json([
                'success' => false,
                'message' => 'Аффилейт и брокер обязательны'
            ], 400);
        }
        
        $capAnalysisService = new \App\Services\CapAnalysisService();
        $history = $capAnalysisService->getMatchKeyHistory($affiliate, $broker, $geos);
        ...
        \Log::error('Cap match history failed', [
            'error' => $e->getMessage(),
            'trace' => $e->getTraceAsString()
        ]);
