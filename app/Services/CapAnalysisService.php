...
class CapAnalysisService
{
    ...
    private function parseStandardCapMessage($messageText)
    {
        ...
        // Правильная логика валидации количества cap/geo:
        // 1. Если одно поле имеет 1 значение, а другое несколько - ОК (одно значение применяется ко всем)
        // 2. Если оба поля имеют одинаковое количество значений - ОК
        // 3. Только если оба поля имеют разное количество И ни одно не равно 1 - отклоняем
        $capCount = count($caps);
        $geoCount = count($geos);
        
        if ($capCount !== $geoCount && $capCount !== 1 && $geoCount !== 1) {
            return null; // Отклоняем только если оба поля имеют разное количество и ни одно не равно 1
        }
        ...
        // Правильная логика для обработки разного количества cap/geo:
        // Определяем максимальное количество между caps и geos
        $count = max(count($caps), count($geos));
        
        // Если cap имеет 1 значение, а geo несколько - размножаем cap
        if (count($caps) === 1 && count($geos) > 1) {
            $caps = array_fill(0, $count, $caps[0]);
        }
        
        // Если geo имеет 1 значение, а cap несколько - размножаем geo  
        if (count($geos) === 1 && count($caps) > 1) {
            $geos = array_fill(0, $count, $geos[0]);
        }
