...
<head>
    ...
    <style>
        * {
        ...
        .history-change-value.old {
            color: #ef4444;
        }
        
        .history-change-value.new {
            color: #10b981;
        }
...
<body>
    ...
    <div class="modal-overlay" id="history-modal" style="display: none;">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title" id="history-modal-title">История изменений</h3>
                <button class="close-btn" onclick="closeHistoryModal()">Закрыть</button>
            </div>
    ...
    <script>
                    ...
                    <div class="message-item">
                        <div class="message-header">
                            ...
                            <div class="message-actions">
                                <button class="history-btn" onclick="showCapHistory('${msg.id}', '${msg.chat_name}')">📜 История</button>
                            </div>
                        </div>
        ...
        // Показать историю изменений для капы
        async function showCapHistory(messageId, chatName) {
            const modal = document.getElementById('history-modal');
            const modalTitle = document.getElementById('history-modal-title');
            const historyContent = document.getElementById('history-content');
            
            if (!modal || !modalTitle || !historyContent) {
                console.error('Элементы модального окна не найдены');
                return;
            }
            
            // Извлекаем capId из messageId (формат: "message_id_cap_id")
            const capId = messageId.split('_')[1]; // Получаем cap_id из составного ID
            
            modalTitle.textContent = `История изменений - ${chatName}`;
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.success) {
                        renderHistoryData(data.history);
                    } else {
                        showHistoryError(data.message || 'Ошибка загрузки истории');
                    }
                } else {
                    showHistoryError('Ошибка сервера');
                }
                
            } catch (error) {
                console.error('Ошибка загрузки истории:', error);
                showHistoryError('Ошибка соединения');
            }
        }
                ...
                <div class="history-item">
                    ...
                    <div class="history-info">
                        <div style="margin-bottom: 0.5rem;">
                            <strong>Источник:</strong> ${entry.source_chat}
                        </div>
                        <div style="margin-bottom: 0.5rem;">
                            <strong>Обновлено:</strong> ${entry.target_chat}
                        </div>
                    </div>
                        ...
                        <div class="history-changes">
                            ${entry.changed_fields.map(field => `
                                <div class="history-change">
                                    <div class="history-change-field">${getFieldDisplayName(field)}</div>
                                    <div class="history-change-value old">Было: ${formatHistoryValue(entry.old_values[field])}</div>
                                    <div class="history-change-value new">Стало: ${formatHistoryValue(entry.new_values[field])}</div>
                                </div>
                            `).join('')}
                        </div>
                    ` : ''}
        ...
        // Получение человекочитаемых названий полей
        ...
        function showHistoryError(message) {
            const historyContent = document.getElementById('history-content');
            historyContent.innerHTML = `<div class="no-history">❌ ${message}</div>`;
        }
