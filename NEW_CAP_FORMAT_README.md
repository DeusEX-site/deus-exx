...
**Пустые поля**: Поля могут отсутствовать, быть пустыми или содержать "-" (все это считается пустотой)

1. **Total**: 
   - Если пустое или "-" → бесконечность (-1)
   - Если в Cap есть слеш (например `Cap: 15/150`) → Total = бесконечность, Cap = первое число

2. **Date**: 
   - Если пустое или "-" → бесконечность (null)
   - Без года (24.02) → добавляется текущий год (24.02.2024)

3. **Schedule**: 
   - Если пустое или "-" → 24/7

4. **Language**: 
   - Если пустое или "-" → "en"

5. **Funnel**: 
   - Если пустое или "-" → null (может быть пустым)

6. **Pending ACQ / Freeze status**: 
   - Yes/True/1/Да → true
   - Пустое, "-" или остальное → false
...
### Пример 5: Пустые поля и значения "-"
```
Affiliate: EmptyTest
Recipient: TestEmpty
Cap: 10
...
Freeze status on ACQ: -
```
Результат: Cap=10, Total=бесконечность ("-"), Language="en" (по умолчанию), Schedule=24/7 (по умолчанию), Date=бесконечность ("-"), Funnel=null, Pending ACQ=false, Freeze=false
